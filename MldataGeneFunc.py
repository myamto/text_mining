...
def goji_datsuji(text):  #誤字脱字
    ...
    def comejisyo_tikan(text): #テキスト内にあるComeJisyoの単語を’ケガ’に置換する
        target=[]
        ...
        for w in target:
            text = text.replace(w, "ケガ")

        return text

    tikan_text=comejisyo_tikan(text)
    each_500_char_list=[]
    sentences=""
    for stc in tikan_text.split("。"):
        if len(sentences + stc) < 500:
